...
import sys
sys.path.append('./PythonScripts/vision/references/detection/')
...
class Dataset(object):
    ...
    def get_label_name(self, label):
        for replacement in label_name_replacements:
            if label in replacement[0]:
                return replacement[1]
        
        return label

    def extract_labels_data(self, shape, image_height, image_width):
        try:
            ################
            # get label id #
            ################
            label_name = shape["label"]
            label_name = self.get_label_name(label_name)
            ...
            ymin = np.min(pos[0])
            ymax = np.max(pos[0])
            xmax = xmax + 0.0001 if xmax <= xmin else xmax
            ymax = ymax + 0.0001 if ymax <= ymin else ymax
            box = [xmin, ymin, xmax, ymax]

            # append results
            return box, mask, label_index

        except:
            return None

    def extract_other_labels_data(self, shapes_to_remove, image_height, image_width):
        try:
            ################
            # get label id #
            ################
            ...
            for rm_shape in shapes_to_remove:
                ...
                mask = self.generate_mask(mask, rm_points, 1)
    ...
    def __getitem__(self, idx):
        ...
        # divide shapes
        shapes = [s for s in label["objects"] if self.get_label_name(s["label"]) in label_names]
        shapes_to_remove = [s for s in label["objects"] if self.get_label_name(s["label"]) not in label_names]
        ...
        for shape in shapes:
            results = self.extract_labels_data(shape, image_height, image_width)

            if results is not None:
                box, mask, label_index = results
                boxes.append(box)
                masks.append(mask)
                labels.append(label_index)

        results = self.extract_other_labels_data(shapes_to_remove, image_height, image_width)
        ...
        iscrowd = torch.zeros((len(shapes)+1,), dtype=torch.int64)
